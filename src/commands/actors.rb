...
module RME
  module Command
    module Actors
      ...
      # ------------------------------------------------------------------------
      # * Updates graphics (i.e.: charset & face) which represents the given
      #   actor; and refreshes the sprites to show them in game.
      # ------------------------------------------------------------------------
      Command::declare({:section     => self,
                        :name        => :actor_change_appear,
                        :description => 'Actors.actor_change_appear',
                        :parameters  => [
                          ACTOR_ID,
                          CHARSETS_FILENAME,
                          CHARSET_INDEX,
        ...
        ::Command.actor_set_graphic(charsets_filename, charset_index, faceset_filename, face_index)
        $game_player.refresh
      end

      # ------------------------------------------------------------------------
      # * Updates the charset which represents the given actor;
      #   and refreshes the sprites to show them in game.
      # ------------------------------------------------------------------------
      Command::declare({:section     => self,
                        :name        => :actor_change_character,
                        :description => 'Actors.actor_change_character',
                        :parameters  => [
                          ACTOR_ID,
                          CHARSETS_FILENAME,
                          CHARSET_INDEX
                        ]}) do |id, charsets_filename, charset_index|
        $game_actors[id].character_name = charsets_filename
        $game_actors[id].character_index = charset_index
        $game_player.refresh
      end
